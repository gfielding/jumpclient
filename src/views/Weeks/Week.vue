    ...
    <div class="dashboard__container pt-3" v-if="eventInfo.id">
      <div class="dashboard__container--header">
        <div class="flex align-center justify-space-between" style="width:100%;">
          <h4 style="width:auto;">Edit Job Week</h4>
        ...
        <div class="dashboard__container--body">
          ...
          <div class="dashboard__container--body--col" style="background: transparent;">
            <div class="flex justify-space-between">
              ...
              <button class="btn btn__success btn__large" @click="updateWeek()" :disabled="event.status == 'cancelled'">
              Update Event
                <transition name="fade">
...
export default {
  ...
  computed: {
    ...mapState(['currentUser', 'weekInfo', 'venues', 'clients', 'jobs', 'mgrs', 'tags', 'groups']),
    event() {
  ...
  methods: {
    addGroup (item) {
      ...
      this.$store.dispatch('updateWeek', event)
    },
    removeGroup (item) {
      console.log(item)
      let event = this.event
    ...
    venueSync() {
      ...
      .then(doc => {
        ...
        this.$store.dispatch('updateWeekJobs', {
    ...
    sheets() {
      let url = `/jobs/` + this.$route.params.id + `/timesheets`
      router.push(url)
    },
    shifts() {
      let url = `/jobs/` + this.$route.params.id + `/shifts`
      router.push(url)
    },
    placements() {
    ...
    email() {
      let url = `/jobs/` + this.$route.params.id + `/email`
      router.push(url)
    },
    checkIn() {
      let url = `/jobs/` + this.$route.params.id + `/checkin`
      router.push(url)
    },
    files() {
      let url = `/jobs/` + this.$route.params.id + `/files`
      router.push(url)
    },
    deleteEvent() {
      let event = this.eventInfo
    ...
    updateVenue() {
      ...
      this.$store.dispatch('updateWeekVenue', event)
      let logFields = {
          staffMember: this.currentUser.email,
    ...
    updateStaff() {
      let event = this.event
      this.$store.dispatch('updateWeekStaff', event)
      let logFields = {
          staffMember: this.currentUser.email,
          ...
          blob => {
            ...
            picRef.put(blob).then((snap) => {
              picRef.getDownloadURL().then(function(croppaURL) {
                ...
                store.dispatch('updateWeek', event)
              })
            })
          }
        )
  ...
  beforeDestroy () {
    ...
    this.$store.dispatch('clearWeekState')
    console.log(this)
  }
}
</script>